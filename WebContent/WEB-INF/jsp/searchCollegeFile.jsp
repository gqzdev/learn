...
<html>
	<head>
		<title>查询课程文件  在线学习平台ForFuture</title>
...
<div id="courseFileContainer" v-cloak>
	<div class="row">
        <div class="col-lg-12">
		...
		<div class="col-lg-12">
			<div class="panel panel-default  wk-panel">
				<table class="table table-striped table-hover">
					...
					<tbody>
						<tr v-for="(courseFile,index) in courseFileList" class="showStyle">
							<td>{{index+1+(page-1)*pageSize}}</td>
							<td>
								<a href="javascript:void()" v-on:click="download(courseFile.path)">{{courseFile.path}}</a>
							</td>
							<td>{{courseFile.courseName}}</td>
							<c:choose>
								<c:when test="${sessionScope.role==0}">
									<td>{{courseFile.teacherName}}</td>
								</c:when>
							</c:choose>
							<c:choose>
								<c:when test="${sessionScope.role==2}">
							...
							<td>{{dateFormat(courseFile.recordTime)}}</td>
							<c:choose>
								<c:when test="${sessionScope.role==0}">
			...
			methods:{
				pageHandler:function(page){//跳转到page页
					...
					if((content==null||content=="")&&courseId==0){
						this.$http.post("courseFileListByPage.action",params,{emulateJSON:true}).then(
							function(res){
								this.courseFileList=res.data.list;
						...
						this.$http.post("courseFileListByTerm.action",paramsTerm,{emulateJSON:true}).then(
							function(res){
								this.courseFileList=res.data.list;
								this.total=res.data.total;
								this.maxPage=res.data.maxPage;
				...
				download:function(path){
					 location.href = "/upload/learn/" + path;
				},
